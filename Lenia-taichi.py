...
class Taichi_Lenia:
    def __init__(self, res, scatter, conv_r, time, miu, sig, kr=1, kb=ti.Vector([1])):
        ...
        self.pixels = ti.Vector.field(3, ti.f32, (res*scatter,res*scatter))
    ...
    def kernel_core(self, r):
        return ti.exp(self.kernel_alpha - self.kernel_alpha/(4.0*r*(1.0-r)))
    ...
    def kernel_shell(self, x, y):
        ...
        for i in ti.static(range(self.kernel_rank)):
            if ti.floor(br) == i:
                c = self.clip((r < 1) * self.kernel_beta[i] * self.kernel_core(br - i), 0.0, 1.0)
        return c
    ...
    def growth_mapping(self, u):
        return 2.0 * ti.exp(-(u - self.grow_miu[None])**2.0/(2.0*(self.grow_sig[None]**2.0))) - 1.0
    ...
    def world_conv(self):
        for i, j in ti.ndrange(self.res, self.res):
            ...
            for m, n in ti.ndrange(2* self.max_conv_r, 2* self.max_conv_r):
                tmp += self.kernel[m, n] * self.world_old[(i-self.max_conv_r + m)%self.res, (j-self.max_conv_r + n)%self.res]
    ...
    def world_update(self):
        for i, j in ti.ndrange(self.res, self.res):
            t = 0.0
            t = self.clip(self.dt * self.world_new[i, j] +  self.world_old[i, j], 0.0, 1.0)
    ...
    def init(self):
        ...
        self.render()
        print(
            "Current parameter: kernel radius:{}, dt:{}, miu:{}, sig:{}, kr:{}, kb:{}".format(
                self.conv_r, 
                self.dt,
                self.grow_miu[None],
                self.grow_sig[None],
